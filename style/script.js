// Optimized settings for smooth performance
  ...
  function onResize() {
    ...
    // ensure the question wrapper is centered above the canvas heart
    if (questionWrapper) {
      const rect = canvas.getBoundingClientRect();
      const cx = rect.left + rect.width / 2;
      const cy = rect.top + rect.height / 2;
      // offset (negative = move up). Change this value to tweak vertical position.
      const offsetY = -40; // px
      questionWrapper.style.left = cx + 'px';
      questionWrapper.style.top = (cy + offsetY) + 'px';
      // keep translate(-50%,-50%) for perfect centering
  ...
  (function(){
    ...
    if (answerBtn) {
      answerBtn.addEventListener('click', function(){
        ...
        // Hide original question text slightly to emphasize typing
        questionText.style.opacity = 0.6;

        // Show hidden text element and type into it, then replace button
        hiddenText.style.display = 'block';
        hiddenText.textContent = '';

        // Replace button visual with disabled state while typing
        answerBtn.disabled = true;
        answerBtn.style.opacity = 0.6;

        typeText(hiddenText, 'Answer on dm discord sweetie', 80, () => {
          // After typing, remove the button and keep text
